...
it('can add the star name and star symbol properly', async () => {
	...
	let instance = await StarNotary.deployed();
	let starId = 3;
	await instance.createStar('Orkut', starId, { from: accounts[0] });

	assert.equal(await instance.tokenIdToStarInfo.call(starId), 'Orkut');
	assert.equal(await instance.name(), 'Galaxy Tokens');
	assert.equal(await instance.symbol(), 'GTK');
});
...
it('lets 2 users exchange stars', async () => {
	...
	await instance.createStar('GooglePlus', starId1, { from: accounts[0] });
	await instance.createStar('Myspace', starId2, { from: accounts[1] });
...
it('lets a user transfer a star', async () => {
	...
	let instance = await StarNotary.deployed();
	let starId = 6;
	await instance.createStar('Friendster', starId, { from: accounts[0] });
